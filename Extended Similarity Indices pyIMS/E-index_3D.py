...
folder = 'Results/04-17-2019_MB_Imaging_raw_data/RR_results/NAPCA-local/'

# Define save path for E index plots
save_path = 'Results/04-17-2019_MB_Imaging_raw_data/plots/E-index/NAPCA-local/'
...
for it_subfolder in os.listdir(folder):
    ...
    for file in os.listdir(it_subfolder_path):
        ...
        w_E_n[file] = E_index(dictionary, include = 5, omit = [], E_type = 'max', weight = 'squared_sum')
        w_E_f[file] = E_index(dictionary, include = 5, omit = [], E_type = 'max', weight = 'fraction')
        w_E_r[file] = E_index(dictionary, include = 5, omit = [], E_type = 'robust', weight = 'weight_PC')
        w_E_rn[file] = E_index(dictionary, include = 5, omit = [], E_type = 'robust', weight = 'squared_sum')
        w_E_rf[file] = E_index(dictionary, include = 5, omit = [], E_type = 'robust', weight = 'fraction')
...
# iterate across all_E dictionary to plot all 6 tested E-index values.
for E_type in all_E:
    fig = plt.figure(E_type, dpi=200)
    ax = plt.axes(projection = '3d')
    
    for it_subfolder, y in zip(all_E[E_type], ys):
        plt.plot(xs, np.array(y).repeat(30), list(all_E[E_type][it_subfolder].values()))
        plt.title(f'{E_type}')
        ax.set_zlabel('E-index')
        ax.set_ylabel('Intensity Threshold')
        ax.set_xlabel('Selected Pixel Percent')
        plt.grid(True)
        ax.view_init(14, -45)

        plt.savefig(f'{save_path}{title}_{E_type}-3D', bbox_inches = 'tight')

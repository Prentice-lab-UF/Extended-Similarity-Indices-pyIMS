# -*- coding: utf-8 -*-
"""
Created on Sat Sep  3 20:03:36 2022
...
for px, PC in enumerate(medoid_indices):
    ...
    for rx,region in enumerate(medoid_indices[PC]):
        ...
        axs[rx].plot(mz_range, total_spectra[region[0]], color = 'b')
        
        # Replot the peaks that were assigned as 1's in the binary fingerprint in red
        axs[rx].plot(mz_range, (total_fp[region[0]]*total_spectra[region[0]]), color = 'r')
        
        # Clean up baseline of spectra 
        axs[rx].plot(mz_range, np.zeros(len(mz_range)), color = 'b')
        
        # If intensity threshold is non-zero plot a grey line to show where the intensity threshold is located
        axs[rx].plot(mz_range, (np.max(total_spectra[region[0]])*it).repeat(len(mz_range)), color = 'grey')
        
        axs[rx].set(ylabel='Intensity (a.u.)')
    ...
    for mz,l in loadings[px,:,:]:
        ...
        # Plot pseudo-spectra
        axs[3].plot(mz_range, yloadings, color = 'b')
        axs[3].set(ylabel='Intensity (a.u.)')
